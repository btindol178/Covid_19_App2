...
app.layout = html.Div(
    [
        dbc.Row(
            [
                dbc.Col(html.Div(dcc.Dropdown(id='stateselector', options=get_options(df['state'].unique()),
                                                      multi=True, value=[df['state'].sort_values()[0]],
                                                      style=dict(width='40%',verticalAlign="middle"),#,backgroundColor = '#243745'), # This chages to 40 % of firstcol for that row
        ...
        dbc.Row(
            [
                dbc.Col(html.Div(dcc.Graph(id='timeseries', config={'displayModeBar': False}, animate=True))),
                dbc.Col(html.Div("Two of Two columns")),
                dbc.Col(html.Div("Three of Two columns")),
            ],
            align="center",
